...
const  calculate = (user, memesInfo)  => {
  ...
  let lulzierMemes = user.memes.filter((meme) => {
    for (const i in lulzier) {
      if (meme._id === lulzier[i]._id) return true;
    }
    return false;
  });
  let dankerMeme = user.memes.filter((meme) => (meme._id === danker[0]._id));

  let lulzierMeme;
  if (lulzier.length < 2) {
    lulzierMeme = lulzierMemes[0] || Object.assign(
      {}, empty, { _id: lulzier._id }
    );
  } else {
    lulzierMemes.forEach((meme) => {
